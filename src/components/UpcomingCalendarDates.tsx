...
export default function UpcomingCalendarDates({ products, orderClass = '' }: UpcomingCalendarDatesProps) {
  ...
  return (
    <section className={`${orderClass} mx-auto w-full max-w-[1440px] px-4 py-9 sm:px-6 sm:py-12 lg:px-10`} aria-labelledby="proximas-datas">
      <div className="rounded-[2rem] border border-slate-200 bg-gradient-to-br from-white via-indigo-50/70 to-violet-50 p-6 shadow-lg shadow-indigo-950/5 sm:p-8">
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {events.map((item) => {
            ...
            return <CampaignTrackedLink key={item.tag} href={href} tag={item.tag} surface="homepage_upcoming" className="group flex min-h-44 flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-indigo-300 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-700">
