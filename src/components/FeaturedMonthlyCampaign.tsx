        ...
        <div className="relative bg-violet-950 text-white">
          ...
          <div className="p-6 sm:p-8">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div><p className="text-xs font-black uppercase tracking-[.14em] text-indigo-700">Materiais vinculados ao tema</p><h3 className="mt-1 text-xl font-black text-slate-950">{featuredThemeProducts.length ? 'Escolhas para começar agora' : 'A campanha está pronta para receber materiais'}</h3><p className="mt-2 text-sm text-slate-600">{featuredThemeProducts.length ? `${featuredThemeProducts.length} ${featuredThemeProducts.length === 1 ? 'material encontrado' : 'materiais encontrados'} para ${featuredThemeName}.` : 'Os materiais aparecerão aqui assim que forem marcados com este tema.'}</p></div>
              <Link href={`/buscar?data=${encodeURIComponent(featuredThemeSearchTerm)}`} className="text-sm font-black text-indigo-700 hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-700">Ver todos →</Link>
            </div>
            {featuredThemeProducts.length ? <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(250px,.55fr)]"><div className="grid grid-cols-1 gap-5 min-[720px]:grid-cols-2">{featuredThemeProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div><aside className="flex flex-col justify-between rounded-2xl border border-indigo-100 bg-indigo-50/70 p-5"><div><p className="text-xs font-black uppercase tracking-[.14em] text-indigo-700">Por que este material?</p><h4 className="mt-2 text-lg font-black text-slate-950">Planeje com mais segurança</h4><p className="mt-3 text-sm leading-6 text-slate-700">Este material foi marcado pelo criador com <strong>{featuredThemeName}</strong>. Veja o formato, a loja, o preço e as condições diretamente no card.</p><dl className="mt-5 space-y-3 text-sm"><div className="flex items-center justify-between gap-3 border-b border-indigo-100 pb-2"><dt className="text-slate-600">Data temática</dt><dd className="font-bold text-indigo-900">{dateLabel}</dd></div><div className="flex items-center justify-between gap-3 border-b border-indigo-100 pb-2"><dt className="text-slate-600">Categoria</dt><dd className="font-bold text-indigo-900">{featuredCalendarEvent?.kind || 'Escolar'}</dd></div><div className="flex items-center justify-between gap-3"><dt className="text-slate-600">Gratuitos</dt><dd className="font-bold text-emerald-700">{featuredThemeFreeProducts.length}</dd></div></dl></div><Link href={`/buscar?data=${encodeURIComponent(featuredThemeSearchTerm)}`} className="mt-6 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-700 px-4 text-sm font-black text-white hover:bg-indigo-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-700">Explorar todos deste tema <ChevronRight className="h-4 w-4" /></Link></aside></div> : <p className="mt-5 rounded-2xl border border-dashed border-indigo-200 bg-indigo-50 p-5 text-sm leading-6 text-slate-600">Quando um criador marcar um material com este tema, ele aparecerá aqui automaticamente. Enquanto isso, você pode explorar a busca temática para encontrar conteúdos próximos.</p>}
