...
export async function GET(request: Request) {
  try {
    ...
    const { data: pixKeys } = pixKeyIds.length
      ? await supabaseAdmin.from('creator_pix_keys').select('id, pix_key, pix_key_type, holder_name, bank_name').in('id', pixKeyIds)
      : { data: [] };
    ...
    const enrichedWithdrawals = (withdrawals || []).map((item: any) => ({
      ...
      holder_name: pixKeysById.get(item.pix_key_id)?.holder_name || null,
      bank_name: pixKeysById.get(item.pix_key_id)?.bank_name || null
    }));
