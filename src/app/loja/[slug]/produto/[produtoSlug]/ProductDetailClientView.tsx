...
export default function ProductDetailClientView({ 
  ...
  const creatorBlockReturnFocus = useRef<HTMLElement | null>(null);
  const creatorBlockCloseButton = useRef<HTMLButtonElement | null>(null);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setShowCreatorBlockModal(false);
    };

    document.addEventListener('keydown', handleKeyDown);
    creatorBlockCloseButton.current?.focus();

    return () => {
      document.removeEventListener('keydown', handleKeyDown);
      creatorBlockReturnFocus.current?.focus();
    };
  ...
  const handleClaimFreeMaterial = async () => {
    ...
    try {
      ...
      if (session.role === 'creator') {
        creatorBlockReturnFocus.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
      ...
      setPurchaseError(error instanceof Error ? error.message : 'Não foi possível liberar o material.');
    } finally {
      setIsBuying(false);
    }
  };
  ...
  const handleAddOnly = async () => {
    ...
    try {
      ...
      setIsBuying(false);
      setPurchaseError('Não foi possível adicionar este material ao carrinho. Tente novamente.');
    }
  };
  ...
  const handleStartCheckout = async () => {
    ...
    try {
      ...
      setIsBuying(false);
      setPurchaseError('Não foi possível iniciar a compra. Tente novamente.');
    }
  };
      ...
      {context !== 'marketplace' && (
        <>
          <div className="hidden sm:flex bg-slate-900 py-2 px-4 text-center text-xs text-slate-300 items-center justify-center gap-2">
            ...
            <span>Compra e acesso organizados pela Educalizando</span>
          </div>
          ...
          <div className="min-w-0 lg:col-span-7 xl:col-span-8 space-y-6 sm:space-y-8">
            ...
            <div className="bg-white p-4 sm:p-6 rounded-3xl border border-slate-200/80 shadow-md flex flex-col gap-4">
              ...
              <div className="aspect-[3/4] max-w-md mx-auto w-full rounded-2xl overflow-hidden bg-slate-100 relative shadow-inner">
                {activeMedia ? (
                  <AnimatePresence mode="wait">
                    {activeMedia.type === 'image' ? <motion.img
                      ...
                      onError={() => setFailedImageUrls((urls) => urls.includes(activeMedia.url) ? urls : [...urls, activeMedia.url])}
                ...
                {activeMedia?.type === 'image' && failedImageUrls.includes(activeMedia.url) && (
                  <div className="absolute inset-0 flex items-center justify-center bg-slate-100 p-8 text-center text-sm font-semibold text-slate-500">
                    A imagem de prévia não pôde ser carregada.
                  </div>
                )}
              </div>
            ...
            <section className="lg:hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
              ...
              <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-100">
                ...
                <div className="mt-1 flex items-center gap-2">
                  <div><>{originalPrice && <span className="block text-xs font-bold text-slate-400 line-through">R$ {originalPrice.toFixed(2).replace('.', ',')}</span>}</><strong className="text-3xl font-black tracking-tight text-slate-900">R$ {isFreeProduct ? 'Grátis' : currentPrice.toFixed(2).replace('.', ',')}</strong></div>
                  {originalPrice && <span className="rounded-md bg-emerald-100 px-2 py-1 text-[10px] font-bold text-emerald-700">Preço promocional</span>}
                </div>
              </div>
            ...
            {/* Benefits Box */}
            {hasConfiguredDelivery && <div className="bg-gradient-to-br from-blue-900 via-indigo-950 to-slate-900 p-6 sm:p-8 rounded-3xl text-white shadow-xl space-y-6 relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
                ...
                <div className="space-y-2 flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    ...
                    <span className="bg-slate-100 text-slate-600 text-[10px] font-bold px-2.5 py-0.5 rounded-full border border-slate-200 inline-flex items-center gap-1 self-center">
                      Criador na Educalizando
                    </span>
                  </div>
                  ...
                  <Link href={`/loja/${store.slug}`} className="inline-flex min-h-11 items-center rounded-xl px-3 text-xs font-black text-blue-700 hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600">
                    Ver vitrine de {store.nome_loja}
                  </Link>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white shadow-[0_10px_40px_rgb(0,0,0,0.06)] rounded-3xl p-6 sm:p-8 flex flex-col space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-baseline gap-2 flex-wrap">
                  ...
                  {originalPrice && <span className="text-xs text-emerald-700 font-bold bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">Preço promocional</span>}
                </div>
                <p className="text-xs text-slate-500 font-medium mt-2">Valor da modalidade selecionada.</p>
              </div>
