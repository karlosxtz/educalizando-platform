...
export default function CreatorToolsPage() {
  ...
  const processPdf = async (event: ChangeEvent<HTMLInputElement>, action: 'join' | 'split') => {
    ...
    try {
      ...
      else { const source = await PDFDocument.load(await files[0].arrayBuffer()); const zip = new JSZip(); for (let index = 0; index < source.getPageCount(); index++) { const output = await PDFDocument.create(); output.addPage((await output.copyPages(source, [index]))[0]); zip.file(`pagina-${index + 1}.pdf`, await output.save()); } download(await zip.generateAsync({ type: 'blob' }), `${files[0].name.replace(/\.pdf$/i, '')}-paginas-separadas.zip`); }
      setNotice(action === 'split' ? 'Páginas separadas em um arquivo ZIP pronto para baixar.' : 'PDF unido e pronto para baixar.');
