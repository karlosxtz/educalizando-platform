...
import { Store, Search, Filter, AlertCircle, ShoppingBag, DollarSign, Loader2, CheckCircle2 } from 'lucide-react';
import { getAvailableMarketplaceProducts, applyForProductAffiliation, getMyAffiliations } from '@/lib/affiliate-service';
...
export default function AffiliateMarketplacePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [marketProducts, myAff] = await Promise.all([
