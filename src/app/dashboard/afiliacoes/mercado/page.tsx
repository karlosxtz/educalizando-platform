...
export default function AffiliateMarketplacePage() {
  ...
  const [myAffiliations, setMyAffiliations] = useState<Affiliate[]>([]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          ...
          className="min-h-11 w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600"
        >
          Minhas Afiliações
        </Link>
      </div>
      ...
      {successMsg && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-xl text-sm font-bold flex items-center gap-2" role="status" aria-live="polite">
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-3">
        ...
        <label htmlFor="affiliate-market-search" className="sr-only">Buscar produto ou loja</label>
        <input 
          id="affiliate-market-search"
          type="text"
          placeholder="Buscar produto ou loja..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="min-h-11 flex-1 bg-transparent border-none outline-none text-sm font-semibold text-slate-900 placeholder:text-slate-400 focus-visible:ring-2 focus-visible:ring-teal-600 rounded-lg"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-12 text-center shadow-sm">
                ...
                {filtered.map(product => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="min-w-0 bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col"
                    >
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        <h3 className="font-bold text-slate-900 text-lg line-clamp-2 mb-1" title={product.titulo}>{product.titulo}</h3>
                        ...
                        <div className="mt-5 pt-5 border-t border-slate-100">
                          {productAffiliate ? (
                            <div className="flex flex-col gap-2">
                              ...
                              {productAffiliate.status === 'rejeitado' && (
                                <button
                                  ...
                                  className="min-h-11 text-xs font-bold text-brand-navy hover:underline transition-colors w-full text-center py-1 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-navy rounded-lg"
                                >
                              ...
                              {productAffiliate.status !== 'rejeitado' && productAffiliate.status !== 'cancelado' && (
                                <button
                                  ...
                                  className="min-h-11 text-xs font-bold text-slate-400 hover:text-red-500 transition-colors w-full text-center py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-600 rounded-lg"
                                >
                                  Cancelar filiação
                                </button>
                              )}
                            ...
                            <button
                              onClick={() => handleAffiliate(product.id, product.store_id)}
                              disabled={affiliatingId === product.id}
                              className="min-h-11 w-full py-2.5 bg-brand-navy hover:bg-brand-navy-hover text-white text-sm font-bold rounded-xl transition-colors shadow-md shadow-brand-navy/20 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-navy focus-visible:ring-offset-2"
                            >
