...
import { 
  ...
  FileText, Video, BookOpen, HelpCircle, Layers, CheckCircle2, Loader2, Sparkles 
} from 'lucide-react';
...
export default function ProductsManagementPage() {
  ...
  const getTipoIcon = (tipo: ProductType) => {
    switch (tipo) {
      ...
      case 'ebook': return <BookOpen className="w-4 h-4 text-indigo-400" />;
      case 'video': return <Video className="w-4 h-4 text-purple-400" />;
      case 'curso': return <Layers className="w-4 h-4 text-[#ff5722]" />;
      case 'simulado': return <HelpCircle className="w-4 h-4 text-amber-400" />;
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white flex items-center gap-3">
            <Package className="w-7 h-7 text-[#ff5722]" /> Meus Produtos Didáticos ({products.length})
          </h1>
        ...
        <button
          ...
          className="px-5 py-3 rounded-xl font-extrabold text-sm bg-gradient-to-r from-[#ff5722] to-[#ea580c] text-white shadow-lg shadow-[#ff5722]/25 hover:shadow-[#ff5722]/40 transition-all flex items-center gap-2"
        >
      ...
      {products.length === 0 ? (
        <div className="glass-panel p-12 text-center max-w-lg mx-auto space-y-4">
          ...
          <h3 className="text-lg font-bold text-white">Nenhum produto cadastrado ainda</h3>
          <p className="text-sm text-slate-400">
            Cadastre seu primeiro e-book ou apostila digital em PDF para começar a vender na sua loja.
          </p>
          <button
            onClick={handleOpenCreateModal}
            className="px-5 py-2.5 rounded-xl font-bold text-xs bg-[#ff5722] text-white inline-flex items-center gap-2"
          >
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map(prod => (
            <div
              key={prod.id}
              className="glass-panel glass-panel-hover p-5 flex flex-col justify-between space-y-4 relative overflow-hidden"
            >
              <div className="space-y-3">
                ...
                <div className="h-40 rounded-xl overflow-hidden bg-slate-800 relative">
                  ...
                  <button
                    ...
                    className={`absolute top-2 right-2 px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider flex items-center gap-1 shadow-md ${
                  ...
                  {/* Type Badge */}
                  <span className="absolute bottom-2 left-2 bg-slate-900/90 text-white text-[10px] font-bold px-2 py-1 rounded-md flex items-center gap-1 border border-white/10 uppercase">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div
              ...
              className="glass-panel border-white/20 w-full max-w-xl p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto"
            >
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-[#0b0f19]/70 border border-white/10 focus:border-[#ff5722] rounded-xl text-white text-sm focus:outline-none"
                  />
                  {errors.titulo && (
                    <p className="text-xs text-rose-400 mt-1 font-medium">{errors.titulo.message}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    />
                    {errors.preco && (
                      <p className="text-xs text-rose-400 mt-1 font-medium">{errors.preco.message}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-4 flex justify-end gap-3 border-t border-white/10">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="px-4 py-2.5 rounded-xl font-bold text-xs bg-white/5 hover:bg-white/10 text-slate-300"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="px-6 py-2.5 rounded-xl font-bold text-xs bg-[#ff5722] text-white hover:bg-[#ea580c] flex items-center gap-2 disabled:opacity-50"
                  >
