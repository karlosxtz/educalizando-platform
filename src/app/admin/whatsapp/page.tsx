...
export default function WhatsAppAutomationsPage() {
  ...
  const restart = async () => {
    setRestarting(true);
    try {
      const response = await fetch('/api/admin/whatsapp-instance', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'restart' }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      toast.success('Instância reiniciada. Aguarde alguns segundos e faça um novo teste.');
      window.setTimeout(() => { void refreshHealth(true); }, 3000);
    } catch (error: any) {
      toast.error(error.message || 'Não foi possível reiniciar a instância.');
    } finally {
      setRestarting(false);
    }
  };
  ...
  const syncWebhook = async () => {
    ...
    try {
      const response = await fetch('/api/admin/whatsapp-instance', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'sync_webhook' }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      toast.success('Recebimento de comandos ativado. “CANCELAR PAGAMENTO” já funciona neste WhatsApp.');
    } catch (error: any) {
      toast.error(error.message || 'Não foi possível ativar o recebimento de comandos.');
    } finally {
